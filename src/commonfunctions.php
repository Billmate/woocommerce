...
define('BILLPLUGIN_VERSION', '3.12.1');
...
add_action('woocommerce_order_refunded', 'credit_billmate_order', 1);

function cancel_billmate_order($order_id, $action = false){
    $order = wc_get_order($order_id);
    $method = $order->get_payment_method();
    if (strpos(strtolower($method), 'billmate') !== false) {
        if (get_option('billmate_common_cancelonstatus') == 'active' && get_option('billmate_common_activateonstatus') == 'active') {
            ...
            if ($shouldCancel && !$hasBeenActivated) {
                if ($number !== null) {
                    ...
                    if ($methodClass) {
                        ...
                        $billmate = new BillMate(get_option('billmate_common_eid'), get_option('billmate_common_secret', 'billmate'), true, $methodClass->testmode == 'yes', false);
