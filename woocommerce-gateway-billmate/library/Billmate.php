...
/**
 * This API provides a way to integrate with Billmate's services over the XMLRPC protocol.
 *
...
class Billmate {
     ...
     * @var string
     */
    private static $c_addr = "api.billmate.se";
     ...
     * @var string
     */
    private static $live_addr = 'api.billmate.se';
     ...
     * @var string
     */
    private static $beta_addr = 'apitest.billmate.se';
     ...
     *
     * Use the absolute or relative URI to a file if {@link Billmate::$pcStorage} is set as 'xml' or 'json'.<br>
     * Use a HTTP-auth similar URL if {@link Billmate::$pcStorage} is set as 'mysql', <br>
     ...
     * @return void
     */
    public function config($eid, $secret, $country, $language, $currency, $mode = Billmate::LIVE, $pcStorage = 'json', $pcURI = 'billmatepclasses.json', $ssl = true, $candice = true) {
        try { 
    ...
    public function setConfig(&$config) {
        try {
            if($config instanceof ArrayAccess) {
                ...
                throw new Exception('Supplied config is not a BillmateConfig/ArrayAccess object!', 50001);
            }
        }
        catch(Exception $e) {
            $this->config = null;
            throw new BillmateException('Error in ' . __METHOD__ . ': ' . $e->getMessage(), $e->getCode());
        }
    }
     ...
     *
     * @param  int     {@link BillmateCountry Country} constant.
    ...
    public static function getCountryForCode($code) {
        ...
        if ($country === null) {
            throw new BillmateException('Error in ' . __METHOD__ . ': Unknown country! ("'.$code.'")', 50002);
        }
        return $country;
    }
     ...
     *
     * @param  int     {@link BillmateLanguage Language} constant.
    ...
    public static function getLanguageForCode($code) {
        ...
        if ($language === null) {
            throw new BillmateException('Error in ' . __METHOD__ . ': Unknown language! ('.$code.')', 50003);
        }
        return $language;
    }
    ...
    /**
     * Returns the {@link BillmateCurrency currency} constant from the currency code.
     *
    ...
    public static function getCurrencyForCode($code) {
        ...
        if ($code === null) {
            throw new BillmateException('Error in ' . __METHOD__ . ': Unknown currency! ('.$code.')', 50004);
        }
        return $currency;
    }
     ...
     *
     * @param  int     {@link BillmateCurrency Currency} constant.
     ...
     * {@link BillmateCountry} or {@link BillmateCurrency} constants can be used, or letter codes.<br>
     * Uses set values if parameter is null.<br>
     *
     * E.g. Billmate allows Euro with Germany, Netherlands and Finland, thus true will be returned.
     *
     * @param  string|int $country  {@link BillmateCountry}
     * @param  string|int $currency {@link BillmateCurrency}
     * @throws BillmateException
    ...
    public function checkCountryCurrency($country = null, $currency = null) {
        ...
        switch($country) {
            ...
            case BillmateCountry::FI:
                return ($currency !== BillmateCurrency::EUR) ? false : true;
            case BillmateCountry::DK:
                return ($currency !== BillmateCurrency::DKK) ? false : true;
            case BillmateCountry::NO:
                return ($currency !== BillmateCurrency::NOK) ? false : true;
            case BillmateCountry::SE:
                return ($currency !== BillmateCurrency::SEK) ? false : true;
            default:
    ...
    /**
     * Returns the {@link BillmateLanguage language} constant for the specified or set country.
     *
     * @param  int  $country  {@link BillmateCountry Country} constant.
     * @return mixed  False, if no match otherwise {@link BillmateLanguage language} constant.
     */
    ...
    /**
     * Returns the {@link BillmateCurrency currency} constant for the specified or set country.
     *
     * @param  int  $country  {@link BillmateCountry country} constant.
     * @return mixed  False, if no match otherwise {@link BillmateCurrency currency} constant.
     */
    ...
    public function setAddress($type, $addr) {
        if(!($addr instanceof BillmateAddr)) {
            throw new BillmateException('Error in ' . __METHOD__ . ': Supplied address is not a BillmateAddr object!', 50011);
        }
        ...
        else if($type === BillmateFlags::IS_BILLING) {
            $this->billing = $addr;
            self::printDebug("billing address array", $this->billing);
        }
        else {
            throw new BillmateException("Error in " . __METHOD__ . ": Unknown address type ($type)", 50012);
        }
    }
    ...
    protected function assembleAddr($method, $addr) {
        if(!($addr instanceof BillmateAddr)) {
            throw new BillmateException('Error in ' . $method . ': Specified address is not a BillmateAddr object! (Call setAddress first?!)', 50013);
        }

        $tmp = $addr->toArray();

        //Check address!
        if($tmp['country'] === BillmateCountry::NL || $tmp['country'] === BillmateCountry::DE) {
            if(strlen($tmp['house_number']) == 0) {
                throw new BillmateException("Error in " . $method . ': House number needs to be specified for Netherlands and Germany!', 50014);
            }
        }

        if(strlen($tmp['email']) == 0) {
            throw new BillmateException("Error in " . $method . ': Email address not set!', 50015);
        }
        ...
        if(!preg_match("/^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z0-9-][a-zA-Z0-9-]+)+$/", $tmp['email'])) {
            throw new BillmateException("Error in " . $method . ': Email address is not valid! ('.$tmp['email'].')', 50016);
        }

        //Only check fname and lname if company isn't set.
        if(strlen($tmp['company']) == 0) {
            if(strlen($tmp['fname']) == 0) {
                throw new BillmateException("Error in " . $method . ': First name not set!', 50017);
            }

            if(strlen($tmp['lname']) == 0) {
                throw new BillmateException("Error in " . $method . ': Last name not set!', 50018);
            }
        }

        if(strlen($tmp['street']) == 0) {
            throw new BillmateException("Error in " . $method . ': Street address not set!', 50019);
        }

        if(strlen($tmp['zip']) == 0) {
            throw new BillmateException("Error in " . $method . ': Zip code not set!', 50020);
        }

        if(strlen($tmp['city']) == 0) {
            throw new BillmateException("Error in " . $method . ': City not set!', 50021);
        }

        if($tmp['country'] <= 0) {
            throw new BillmateException("Error in " . $method . ': Country not set!', 50022);
        }
    ...
    public function getPNOEncoding() {
        if(!is_int($this->country) || !is_int($this->language) || !is_int($this->currency)) {
            throw new BillmateException('Error in ' . __METHOD__ . ': You must set country, language and currency!', 50023);
        }

        switch($this->country) {
            case BillmateCountry::DE:
                if($this->currency !== BillmateCurrency::EUR || $this->language !== BillmateLanguage::DE) {
                    throw new BillmateException('Error in ' . __METHOD__ . ': Mismatching currency/language for DE! ('.$this->currency.','.$this->language.')', 50024);
                }
                return BillmateEncoding::PNO_DE;
                break;

            case BillmateCountry::DK:
                if($this->currency !== BillmateCurrency::DKK || $this->language !== BillmateLanguage::DA) {
                    throw new BillmateException('Error in ' . __METHOD__ . ': Mismatching currency/language for DK! ('.$this->currency.','.$this->language.')', 50024);
                }
            ...
            case BillmateCountry::FI:
                ...
                if($this->currency !== BillmateCurrency::EUR || ($this->language !== BillmateLanguage::FI && $this->language !== BillmateLanguage::SV)) {
                    throw new BillmateException('Error in ' . __METHOD__ . ': Mismatching currency/language for FI! ('.$this->currency.','.$this->language.')', 50024);
                }
                return BillmateEncoding::PNO_FI;
                break;

            case BillmateCountry::NL:
                if($this->currency !== BillmateCurrency::EUR || $this->language !== BillmateLanguage::NL) {
                    throw new BillmateException('Error in ' . __METHOD__ . ': Mismatching currency/language for NL! ('.$this->currency.','.$this->language.')', 50024);
                }
                return BillmateEncoding::PNO_NL;
                break;

            case BillmateCountry::NO:
                if($this->currency !== BillmateCurrency::NOK || $this->language !== BillmateLanguage::NB) {
                    throw new BillmateException('Error in ' . __METHOD__ . ': Mismatching currency/language for NO! ('.$this->currency.','.$this->language.')', 50024);
                }
                return BillmateEncoding::PNO_NO;
                break;

            case BillmateCountry::SE:
                if($this->currency !== BillmateCurrency::SEK || $this->language !== BillmateLanguage::SV) {
                    throw new BillmateException('Error in ' . __METHOD__ . ': Mismatching currency/language for SE ('.$this->currency.','.$this->language.')', 50024);
                }
                return BillmateEncoding::PNO_SE;
                break;

            default:
                throw new BillmateException('Error in ' . __METHOD__ . ': Unknown/unsupported country! (ISO3166 '.$this->country.')', 50025);
        }
    }
     ...
     * corresponding identifier for {@link Billmate::setIncomeInfo()}, which contains the question,<br>
     * the input type and the values.<br>
     *
     * Note:
     * You need to call {@link Billmate::setAddress()} with {@link BillmateFlags::IS_SHIPPING} before calling this method.
     *
     ...
     * @param  string      $pno       Personal number, SSN, Personal / Corporate , etc.
     * @param  int         $gender    {@link BillmateFlags::FEMALE} or {@link BillmateFlags::MALE}, null or "" for unspecified.
     * @param  int         $encoding  {@link BillmateEncoding Encoding} constant for the PNO parameter.
     * @throws BillmateException
     * @return array
     */
    public function checkILT($amount, $pno, $gender, $encoding = null) {
        if(!is_int($this->country) || !is_int($this->language) || !is_int($this->currency)) {
            throw new BillmateException('Error in ' . __METHOD__ . ': You must set country, language and currency! ('.$this->country.','.$this->currency.','.$this->language.')', 50023);
        }
        ...
        if(!($this->shipping instanceof BillmateAddr)) {
            throw new BillmateException("Error in " . __METHOD__ . ": No shipping address set!", 50035);
        }
        ...
        if($shipping['country'] !== $this->country) {
            throw new BillmateException('Error in ' . __METHOD__ . ': Shipping address country must match the country set!', 50036);
        }
     ...
     */
    public function getAddresses($pno, $encoding = null, $type = BillmateFlags::GA_GIVEN) {

		if($this->country !== BillmateCountry::SE) {
            throw new BillmateException("Error in " . __METHOD__ . ": This method is only available for Swedish customers!", 50025);
        }
       ...
       // self::printDebug("get_addresses result array", $result);
        ...
        foreach($result as $tmpAddr) {
            foreach($tmpAddr as $key => $col){
                    $tmpAddr[$key] = Encoding::fixUTF8(utf8_encode($col));
            }
			
            try {
                $addr = new BillmateAddr();
				$addr->isCompany = empty($tmpAddr[0]) ? true : false;
				if($addr->isCompany) {
					$addr->setCompanyName($tmpAddr[1]);
					$addr->setStreet($tmpAddr[2]);
					$addr->setZipCode($tmpAddr[3]);
					$addr->setCity($tmpAddr[4]);
					$addr->setCountry((int)$tmpAddr[5]);
				}
				else {
     ...
     * @param  int     $flags     Options which specify the article ({@link BillmateFlags::IS_HANDLING}) and it's price ({@link BillmateFlags::INC_VAT})
     * @throws BillmateException
     * @return void
     */
    public function addArticle($qty, $artNo, $title, $price, $vat, $discount = 0, $flags = BillmateFlags::INC_VAT) {
        ...
        try {
            ...
            if($prevException instanceof BillmateException) {
                throw new BillmateException('Error in ' . __METHOD__ . ': Title or ArtNo needs to be set!', 50026);
            }
        }
        ...
        /*if(BillmateFlags::PRINT_10 & $flags) {
     ...
     * {@link Billmate::setShipmentInfo() Billmate::setShipmentInfo('delay_adjust', ...)}<br>
     * with either:<br>
     * {@link BillmateFlags::NORMAL_SHIPMENT NORMAL_SHIPMENT} or {@link BillmateFlags::EXPRESS_SHIPMENT EXPRESS_SHIPMENT}<br>
     *
     * @param  string  $pno            Personal number, SSN, Personal / Corporate , etc.
     * @param  int     $gender         {@link BillmateFlags::FEMALE} or {@link BillmateFlags::MALE}, null or "" for unspecified.
     ...
     */
    public function addTransaction($pno, $gender, $flags = BillmateFlags::NO_FLAG, $pclass = BillmatePClass::INVOICE, $encoding = null, $clear = true) {
        if(!is_int($this->country) || !is_int($this->language) || !is_int($this->currency)) {
            throw new BillmateException('Error in ' . __METHOD__ . ': You must set country, language and currency! ('.$this->country.','.$this->currency.','.$this->language.')', 50023);
        }
        ...
        if(!is_array($this->goodsList) || empty($this->goodsList)) {
            throw new BillmateException("Error in " . __METHOD__ . ": No articles in goods list!", 50034);
        }

        //If only one address is set, copy to the other address.
        if(!($this->shipping instanceof BillmateAddr) && ($this->billing instanceof BillmateAddr)) {
            $this->shipping = $this->billing;
        }
        else if(!($this->billing instanceof BillmateAddr) && ($this->shipping instanceof BillmateAddr)) {
            $this->billing = $this->shipping;
        }
        else if(!($this->billing instanceof BillmateAddr) && !($this->shipping instanceof BillmateAddr)) {
            throw new BillmateException("Error in " . __METHOD__ . ": No address set!", 50035);
        }
    ...
    /**
     * Activates previously created invoice (from {@link Billmate::addTransaction()}).
     *
     * <b>Note</b>:<br>
     * If you want to change the shipment type, you can specify it using:<br>
     * {@link Billmate::setShipmentInfo() Billmate::setShipmentInfo('delay_adjust', ...)}<br>
     * with either: {@link BillmateFlags::NORMAL_SHIPMENT NORMAL_SHIPMENT} or {@link BillmateFlags::EXPRESS_SHIPMENT EXPRESS_SHIPMENT}<br>
     *
     *
     ...
     */
    public function activateInvoice($invNo, $pclass = BillmatePClass::INVOICE, $clear = true) {
    ...
    /**
     * Removes a passive invoices which has previously been created with {@link Billmate::addTransaction()}.<br>
     ...
     *
     * <b>Please note</b>:<br>
     * Activation must be done with activate_reservation, i.e. you cannot activate through Billmate Online.<br>
     *
     * Gender is only required for Germany and Netherlands.<br>
     *
     ...
     *
     * @param  string  $pno        Personal number, SSN, Personal / Corporate , etc.
     * @param  int     $gender     {@link BillmateFlags::FEMALE} or {@link BillmateFlags::MALE}, null for unspecified.
     ...
     */
    public function reserveAmount($pno, $gender, $amount, $flags = 0, $pclass = BillmatePClass::INVOICE, $encoding = null, $clear = true) {
        if(!is_int($this->country) || !is_int($this->language) || !is_int($this->currency)) {
            throw new BillmateException('Error in ' . __METHOD__ . ': You must set country, language and currency!', 50023);
        }
        ...
        if(!is_array($this->goodsList) || empty($this->goodsList)) {
            throw new BillmateException("Error in " . __METHOD__ . ": No articles in goods list!", 50038);
        }
        ...
        if($amount === -1) {
            ...
            if(!is_numeric($amount) || !is_int($amount)) {
                throw new BillmateException("Error in " . __METHOD__ . ": Price not an integer! ($amount)", 50039);
            }
        }
        else {
            $this->checkAmount($amount, __METHOD__);
        ...
        if($amount <= 0) {
            throw new BillmateException("Error in " . __METHOD__ . ": Amount needs to be larger than 0! ($amount)", 50040);
        }
        ...
        if( !($flags & BillmateFlags::RSRV_PHONE_TRANSACTION) ) {
            ...
            if($shipping['country'] !== $this->country) {
                throw new BillmateException('Error in ' . __METHOD__ . ': Shipping address country must match the country set!', 50041);
            }
        }
     ...
     */
    public function changeReservation($rno, $amount, $flags = BillmateFlags::NEW_AMOUNT) {
        $this->checkRNO($rno, __METHOD__);
        $this->checkAmount($amount, __METHOD__);
        $this->checkInt($flags, 'flags', __METHOD__);
     ...
     * {@link Billmate::reserveOCR()} or {@link Billmate::reserveOCRemail()}.<br>
     *
     * <b>Flags can be set to</b>:<br>
     * {@link BillmateFlags::NO_FLAG}<br>
     * {@link BillmateFlags::TEST_MODE}<br>
     ...
     */
    public function activateReservation($pno, $rno, $gender, $ocr = "", $flags = BillmateFlags::NO_FLAG, $pclass = BillmatePClass::INVOICE, $encoding = null, $clear = true) {
        if(!is_int($this->country) || !is_int($this->language) || !is_int($this->currency)) {
            throw new BillmateException('Error in ' . __METHOD__ . ': You must set country, language and currency!', 50023);
        }
        ...
        if(!is_array($this->goodsList) || empty($this->goodsList)) {
            throw new BillmateException("Error in " . __METHOD__ . ": No articles in goods list!", 50043);
        }
        ...
        if( !($flags & BillmateFlags::RSRV_PHONE_TRANSACTION) ) {
            ...
            if($shipping['country'] !== $this->country) {
                throw new BillmateException('Error in ' . __METHOD__ . ': Shipping address country must match the country set!', 50044);
            }
        }
     ...
     */
    public function splitReservation($rno, $amount, $flags = BillmateFlags::NO_FLAG) {
        //Check so required information is set.
        $this->checkRNO($rno, __METHOD__);
        $this->checkAmount($amount, __METHOD__);

        if($amount <= 0) {
            throw new BillmateException("Error in " . __METHOD__ . ": Amount needs to be above 0!", 50045);
        }
    ...
    public function reserveOCR($no, $country = null) {
        ...
        if($country === null) {
            if(!$this->country) {
                throw new BillmateException('Error in' . __METHOD__ . ': You must set country first!', 50046);
            }
            $country = $this->country;
        }
        else {
    ...
    public function reserveOCRemail($no, $email, $country = null) {
        $this->checkNo($no, __METHOD__);
        $this->checkPNO($email, BillmateEncoding::EMAIL, __METHOD__);
        if($country === null) {
            if(!$this->country) {
                throw new BillmateException('Error in' . __METHOD__ . ': You must set country first!', 50046);
            }
    ...
    /**
     * Adds an article number and quantity to be used in {@link Billmate::activatePart()}, {@link Billmate::creditPart()} and {@link Billmate::invoicePartAmount()}.
     *
     ...
     *
     * <b>Note</b>:<br>
     * You need to call {@link Billmate::addArtNo()} first, to specify which articles and how many you want to partially activate.<br><br>
     * If you want to change the shipment type, you can specify it using:<br>
     * {@link Billmate::setShipmentInfo() Billmate::setShipmentInfo('delay_adjust', ...)}<br>
     * with either: {@link BillmateFlags::NORMAL_SHIPMENT NORMAL_SHIPMENT} or {@link BillmateFlags::EXPRESS_SHIPMENT EXPRESS_SHIPMENT}<br>
     *
     ...
     */
    public function activatePart($invNo, $pclass = BillmatePClass::INVOICE, $clear = true) {
        $this->checkInvNo($invNo, __METHOD__);
    ...
    /**
     * Requests a postal send-out of an activated invoice to a customer by Billmate (charges may apply).
     *
     * @param  string  $invNo  Invoice number.
     * @throws BillmateException
     * @return string  Invoice number.
     ...
     * @param  int     $flags   If amount is {@link BillmateFlags::INC_VAT including} or {@link BillmateFlags::NO_FLAG excluding} VAT.
     * @throws BillmateException
     * @return string  Invoice number.
     */
    public function returnAmount($invNo, $amount, $vat, $flags = BillmateFlags::INC_VAT) {
        $this->checkInvNo($invNo, __METHOD__);
        $this->checkAmount($amount, __METHOD__);
        $this->checkVAT($vat, __METHOD__);
        $this->checkInt($flags, 'flags', __METHOD__);
     ...
     * Use this when {@link Billmate::addTransaction()} or {@link Billmate::reserveAmount()} returns a {@link BillmateFlags::PENDING} status.<br>
     *
     * <b>Order status can be</b>:<br>
     * {@link BillmateFlags::ACCEPTED}<br>
     * {@link BillmateFlags::PENDING}<br>
    ...
    public function checkOrderStatus($id, $type = 0) {
        ...
        if(strlen($id) == 0) {
            throw new BillmateException("Error in " . __METHOD__ . ": No ID set!", 50048);
        }

        $this->checkInt($type, 'type', __METHOD__);
        if($type !== 0 && $type !== 1) {
            throw new BillmateException('Error in ' . __METHOD__ . ': Unknown type! ('.$type.')', 50049);
        }
    ...
    public function setCustomerNo($pno, $custNo, $encoding = null) {
        ...
        if(strlen($custNo) == 0) {
            throw new BillmateException("Error in " . __METHOD__ . ": No customer number set!", 50050);
        }
    ...
    public function removeCustomerNo($custNo) {
        ...
        if(strlen($custNo) == 0) {
            throw new BillmateException("Error in " . __METHOD__ . ": No customer number set!", 50051);
        }
    ...
    protected function getPCStorage() {
        require_once('billmatepclasses/storage.intf.php');
        $className = $this->pcStorage.'storage';
        $pclassStorage = dirname(__FILE__).'/billmatepclasses/'.$className.'.class.php';
     ...
     * Removes the cached/stored billmatepclasses and updates.<br>
     ...
     *
     * @param  string|int  $country   {@link BillmateCountry Country} constant, or two letter code.
     * @param  mixed       $language  {@link BillmateLanguage Language} constant, or two letter code.
     * @param  mixed       $currency  {@link BillmateCurrency Currency} constant, or three letter code.
     * @throws BillmateException
     * @return void
     */
    public function fetchPClasses($country = null, $language = null, $currency = null) {
        ...
        if($country === null && $language === null && $currency === null) {
            $countries = array(
                array(
                    'country' => BillmateCountry::DE,
                    'language' => BillmateLanguage::DE,
                    'currency' => BillmateCurrency::EUR
                ),array(
                    'country' => BillmateCountry::DK,
                    'language' => BillmateLanguage::DA,
                    'currency' => BillmateCurrency::DKK
                ),array(
                    'country' => BillmateCountry::FI,
                    'language' => BillmateLanguage::FI,
                    'currency' => BillmateCurrency::EUR
                ),array(
                    'country' => BillmateCountry::NL,
                    'language' => BillmateLanguage::NL,
                    'currency' => BillmateCurrency::EUR
                ),array(
                    'country' => BillmateCountry::NO,
                    'language' => BillmateLanguage::NB,
                    'currency' => BillmateCurrency::NOK
                ),array(
                    'country' => BillmateCountry::SE,
                    'language' => BillmateLanguage::SV,
                    'currency' => BillmateCurrency::SEK
                ),
            );
        }
        else {
        ...
        try {
            if($this->config instanceof ArrayAccess) {
                ...
                foreach($countries as $item) {
                    ...
                    self::printDebug('get_classes array', $paramList);

                    $result = $this->xmlrpc_call('get_pclasses', $paramList);
                    
                    self::printDebug('get_billmatepclasses result', $result);

                    foreach($result as &$pclass) {
                        //numeric htmlentities
						if(strlen( $pclass[1]) <=0) return false;
                        $pclass[1] = utf8_encode($pclass[1]);
                        ...
                        $billmatepclasses->addPClass(new BillmatePClass($pclass));
                    }
                }
                $billmatepclasses->save($this->pcURI);
                $this->billmatepclasses = $billmatepclasses;
            }
            else {
                throw new Exception('Billmate instance not fully configured!', 50001);
            }
        }
        catch(Exception $e) {
            $this->billmatepclasses = null;
            throw new BillmateException('Error in ' . __METHOD__ . ': ' . $e->getMessage(), $e->getCode());
        }
    }
    ...
    public function clearPClasses() {
        if($this->config instanceof ArrayAccess) {
            $billmatepclasses = $this->getPCStorage();
            $billmatepclasses->clear($this->pcURI);
        }
        else {
            throw new BillmateException('Error in ' . __METHOD__ . ': Billmate instance not fully configured!', 50001);
        }
    }
    ...
    public function getPClasses($type = null) {
        try {
            ...
            if(!$this->billmatepclasses) {
                $this->billmatepclasses = $this->getPCStorage();
                $this->billmatepclasses->load($this->pcURI);
            }
            $tmp = $this->billmatepclasses->getPClasses($this->eid, $this->country, $type);
    ...
    public function getPClass($id) {
        try {
            ...
            if(!$this->billmatepclasses || !($this->billmatepclasses instanceof PCStorage)) {
                $this->billmatepclasses = $this->getPCStorage();
                $this->billmatepclasses->load($this->pcURI);
            }
            return $this->billmatepclasses->getPClass($id, $this->eid, $this->country);
        }
        catch(Exception $e) {
            throw new BillmateException('Error in ' . __METHOD__ . ': ' . $e->getMessage(), $e->getCode());
        }
    ...
    public function sortPClasses(&$array) {
        ...
        //Sort billmatepclasses array after natural sort (natcmp)
    ...
    public function getCheapestPClass($sum, $flags) {
        ...
        foreach($this->getPClasses() as $pclass) {
            $lowest_payment = BillmateCalc::get_lowest_payment_for_account($pclass->getCountry());
            if($pclass->getType() < 2 && $sum >= $pclass->getMinAmount()) {
                ...
                if($minpay < $lowest_pp || $lowest_pp === false) {
                    if($pclass->getType() == BillmatePClass::ACCOUNT || $minpay >= $lowest_payment) {
                        $lowest_pp = $minpay;
                        $lowest = $pclass;
                    }
                }
    ...
    public static function digest($data, $hash = null) {
        if($hash===null) {
            ...
            if(count($hashes) == 0) {
                throw new BillmateException("Error in " . __METHOD__ . ": No available hash algorithm supported!");
            }
    ...
    private function checkInvNo(&$invNo, $method) {
        if(!isset($invNo)) {
            throw new BillmateException("Error in " . $method . ": Invoice number not set!", 50055);
        }
        if(!is_string($invNo)) {
            $invNo = strval($invNo);
        }
        if(strlen($invNo) == 0) {
            throw new BillmateException("Error in " . $method . ": Invoice number not set!", 50056);
        }
    }
    ...
    private function checkQty(&$qty, $method) {
        if(!isset($qty)) {
            throw new BillmateException("Error in " . $method . ": Quantity is not set!", 50057);
        }
        if(is_numeric($qty) && !is_int($qty)) {
            $qty = intval($qty);
        }
        if(!is_int($qty)) {
            throw new BillmateException("Error in " . $method . ": Quantity is not an integer! ($qty)", 50058);
        }
    }
    ...
    private function checkArtTitle(&$artTitle, $method) {
        ...
        if(!isset($artTitle) || strlen($artTitle) == 0) {
            throw new BillmateException("Error in " . $method . ": No artTitle specified!", 50059);
        }
    }
    ...
    private function checkArtNo(&$artNo, $method) {
        ...
        if(!isset($artNo) || strlen($artNo) == 0 || (!is_string($artNo))) {
            throw new BillmateException("Error in " . $method . ": No artNo specified! ($artNo)", 50060);
        }
    }
    ...
    private function checkCredNo(&$credNo, $method) {
        if(!isset($credNo)) {
            throw new BillmateException("Error in " . $method . ": Credit number not set!", 50061);
        }
        ...
        if(!is_string($credNo)) {
            ...
            if(!is_string($credNo)) {
                throw new BillmateException("Error in " . $method . ": Credit number couldn't be converted to string! ($credNo)", 50062);
            }
        }
    }
    ...
    private function checkArtNos(&$artNos, $method) {
        if(!is_array($artNos)) {
            throw new BillmateException("Error in " . $method . ": No artNos array specified!", 50063);
        }
        if(empty($artNos)) {
            throw new BillmateException('Error in ' . $method . ': ArtNo array is empty!', 50064);
        }
    }
    ...
    private function checkInt(&$int, $field, $method) {
        if(!isset($int)) {
            throw new BillmateException("Error in " . $method . ": $field is not set!", 50065);
        }
        if(is_numeric($int) && !is_int($int)) {
            $int = intval($int);
        }
        if(!is_numeric($int) || !is_int($int)) {
            throw new BillmateException("Error in " . $method . ": $field not an integer! ($int)", 50066);
        }
    }
    ...
    private function checkVAT(&$vat, $method) {
        if(!isset($vat)) {
            throw new BillmateException("Error in " . $method . ": VAT is not set!", 50067);
        }
        if(is_numeric($vat) && (!is_int($vat) || !is_float($vat))) {
            $vat = floatval($vat);
        }
        if(!is_numeric($vat) || (!is_int($vat) && !is_float($vat))) {
            throw new BillmateException("Error in " . $method . ": VAT not an integer or float! ($vat)", 50068);
        }
    }
    ...
    private function checkAmount(&$amount, $method) {
        if(!isset($amount)) {
            throw new BillmateException("Error in " . $method . ": Amount not set!", 50069);
        }
        ...
        if(!is_numeric($amount) || !is_int($amount)) {
            throw new BillmateException("Error in " . $method . ": Amount not an integer! ($amount)", 50070);
        }
    }
    ...
    private function checkPrice(&$price, $method) {
        if(!isset($price)) {
            throw new BillmateException("Error in " . $method . ": Price is not set!", 50071);
        }
        if(is_numeric($price)) {
            $this->fixValue($price);
        }
        ...
        if(!is_numeric($price) || !is_int($price)) {
            throw new BillmateException("Error in " . $method . ": Price not an integer! ($price)", 50072);
        }
    }
    ...
    private function checkDiscount(&$discount, $method) {
        if(!isset($discount)) {
            throw new BillmateException("Error in " . $method . ": Discount is not set!", 50073);
        }
        if(is_numeric($discount) && (!is_int($discount) || !is_float($discount))) {
            $discount = floatval($discount);
        }
        if(!is_numeric($discount) || (!is_int($discount) && !is_float($discount))) {
            throw new BillmateException("Error in " . $method . ": Discount not an integer or float! ($discount)", 50074);
        }
    }
    ...
    private function checkEstoreOrderNo(&$estoreOrderNo, $method) {
        if(!isset($estoreOrderNo)) {
            throw new BillmateException("Error in " . $method . ": Order number not set!", 50075);
        }
        if(!is_string($estoreOrderNo)) {
            $estoreOrderNo = strval($estoreOrderNo);
            if(!is_string($estoreOrderNo)) {
                throw new BillmateException("Error in " . $method . ": Order number couldn't be converted to string! ($estoreOrderNo)", 50076);
            }
        }
    }
    ...
    private function checkPNO(&$pno, $enc, $method) {
        if(!$pno) {
            throw new BillmateException("Error in " . $method . ": PNO/SSN not set for customer or given as a parameter!", 50077);
        }

        if(!BillmateEncoding::checkPNO($pno, $enc)) {
            throw new BillmateException("Ej giltigt organisations-/personnummer. Kontrollera numret.", 50078);
        }
    }
    ...
    private function checkCountry(&$country, $method) {
        if(!isset($country)) {
            throw new BillmateException("Error in " . $method . ": Country argument not set!", 50079);
        }
        if(is_numeric($country) && !is_int($country)) {
            $country = intval($country);
        }
        if(!is_numeric($country) || !is_int($country)) {
            throw new BillmateException("Error in " . $method . ": Country must be an integer! ($country)", 50080);
        }
    }
    ...
    private function checkLanguage(&$language, $method) {
        if(!isset($language)) {
            throw new BillmateException("Error in " . $method . ": Language argument not set!", 50081);
        }
        if(is_numeric($language) && !is_int($language)) {
            $language = intval($language);
        }
        if(!is_numeric($language) || !is_int($language)) {
            throw new BillmateException("Error in " . $method . ": Language must be an integer! ($language)", 50082);
        }
    }
    ...
    private function checkCurrency(&$currency, $method) {
        if(!isset($currency)) {
            throw new BillmateException("Error in " . $method . ": Currency argument not set!", 50083);
        }
        if(is_numeric($currency) && !is_int($currency)) {
            $currency = intval($currency);
        }
        if(!is_numeric($currency) || !is_int($currency)) {
            throw new BillmateException("Error in " . $method . ": Currency must be an integer! ($currency)", 50084);
        }
    }
    ...
    private function checkNo(&$no, $method) {
        if(!isset($no)) {
            throw new BillmateException("Error in " . $method . ": Argument no not set!", 50085);
        }
        if(is_numeric($no) && !is_int($no)) {
            $no = intval($no);
        }
        if(!is_numeric($no) || !is_int($no) || $no <= 0) {
            throw new BillmateException("Error in " . $method . ": Argument no must be an integer and above 0! ($no)", 50086);
        }
    }
    ...
    private function checkRNO(&$rno, $method) {
        ...
        if(strlen($rno) == 0) {
            throw new BillmateException("Error in " . $method . ": RNO isn't set!", 50087);
        }
    }
    ...
    private function checkRef(&$reference, &$refCode, $method) {
        if(!is_string($reference)) {
            ...
            if(!is_string($reference)) {
                throw new BillmateException("Error in " . $method . ": Reference couldn't be converted to string! ($reference)", 50088);
            }
        }
        ...
        if(!is_string($refCode)) {
            ...
            if(!is_string($refCode)) {
                throw new BillmateException("Error in " . $method . ": Reference code couldn't be converted to string! ($refCode)", 50089);
            }
        }
    }
    ...
    private function checkOCR(&$ocr, $method) {
        if(!is_string($ocr)) {
            $ocr = strval($ocr);
            if(!is_string($ocr)) {
                throw new BillmateException("Error in " . $method . ": OCR couldn't be converted to string or isn't a string! ($ocr)", 50090);
            }
        }
    }
...
/**
 * BillmateException class, only used so it says "BillmateException" instead of Exception.
 *
 * @package BillmateAPI
 */
